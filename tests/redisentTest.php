...
class redisentTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->object = new redisent("localhost", 6379);
    ...
    public function testRedisConnection()
    {
        ...
        $this->assertContains("redis_version", $this->object->info());
    }
    ...
    public function testExceptionHsetOnExistingNormalKey()
    {   
        try {
            $key="key";
            $val= "value";
            $resp = $this->object->hset($key,"a","1");
            $this->assertEquals("(error) ERR Operation against a key holding the wrong kind of value", $resp);
        }
         catch (RedisentException $expected) {
                  return;
              }
    ...
    {  
        ...
        foreach ($keys as $key)  $this->assertEquals($this->object->exists($key),1);
        
        $this->assertEquals($this->object->exists("InExistantKey"),0);
    }
    ...
    {  
        $keys=array("key", "key1","key2", "key3", "hashkey");
        foreach ($keys as $key)  $this->object->del($key);
        foreach ($keys as $key)  $this->assertEquals($this->object->exists($key),0);
    }
